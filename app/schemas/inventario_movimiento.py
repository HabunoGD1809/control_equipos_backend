...
class InventarioMovimientoBase(BaseModel):
    ...
    ubicacion_origen_id: Optional[uuid.UUID] = Field(None, description="ID de la Ubicación de origen.")
    ubicacion_destino_id: Optional[uuid.UUID] = Field(None, description="ID de la Ubicación de destino.")
    ...
    def check_logic_by_type(self) -> 'InventarioMovimientoBase':
        ...
        if tipo in [TipoMovimientoInvEnum.SALIDA_USO, TipoMovimientoInvEnum.SALIDA_DESCARTE, TipoMovimientoInvEnum.AJUSTE_NEGATIVO, TipoMovimientoInvEnum.TRANSFERENCIA_SALIDA, TipoMovimientoInvEnum.DEVOLUCION_PROVEEDOR]:
            if not self.ubicacion_origen_id:
                raise ValueError("ubicacion_origen_id es requerida para este tipo de movimiento.")
        ...
        if tipo in [TipoMovimientoInvEnum.ENTRADA_COMPRA, TipoMovimientoInvEnum.AJUSTE_POSITIVO, TipoMovimientoInvEnum.TRANSFERENCIA_ENTRADA, TipoMovimientoInvEnum.DEVOLUCION_INTERNA]:
            if not self.ubicacion_destino_id:
                raise ValueError("ubicacion_destino_id es requerida para este tipo de movimiento.")
...
class InventarioMovimiento(InventarioMovimientoInDBBase):
    ...
    # Para no romper el frontend, Pydantic extraerá los nombres reales si hiciera falta
    ubicacion_origen_nombre: Optional[str] = None
    ubicacion_destino_nombre: Optional[str] = None

    @model_validator(mode='before')
    @classmethod
    def extract_ubicacion_nombres(cls, data: Any) -> Any:
        if hasattr(data, 'origen') and data.origen:
            data.ubicacion_origen_nombre = data.origen.nombre
        if hasattr(data, 'destino') and data.destino:
            data.ubicacion_destino_nombre = data.destino.nombre
        return data
